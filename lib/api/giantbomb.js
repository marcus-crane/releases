...
const queryMultiple = (title) => {
  return axios.get(`http://www.giantbomb.com/api/search?api_key=${process.env.GIANT_BOMB_API_KEY}&format=json&query=${title}&resources=game&field_list=name,image,original_release_date`, {
    headers: {
      'User-Agent': 'Releases/0.2 (+http://releases.thingsima.de)'
    }
  })
